module.exports = function toReadable(number) {
    ...
    const primeDecimalNumbers = ['ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'ninteen'];
    const decimalNumbers = [, , 'twenty', 'thirty', 'fourty', 'fifty', 'sixty', 'seventy', 'eighty', 'ninety'];
    ...
    if (number.toString().length === 3) {
        return primeNumbers[+number.toString().split('')[0]] + ' ' + hundred + ' ' +
            decimalNumbers[+number.toString().split('')[1]] + ' ' + (+number.toString().split('')[2] !== 0 ?
